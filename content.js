...
const init = () => {
    ...
    const attachViewerAndButtons = (item, preFetchedText, keywordToHighlight) => {
        const safeKeyword = keywordToHighlight ? keywordToHighlight.replace(/"/g, '&quot;').replace(/\\/g, '\\\\') : "";
        const magicScript = `
            <script>
                setTimeout(() => {
        ...
        const openViewer = async (e) => {
            ...
            if (preFetchedText) {
                iframe.srcdoc = preFetchedText + magicScript;
            } else {
                iframe.srcdoc = "<h3 style='font-family:Arial; padding: 20px;'>Carregando arquivo... aguarde.</h3>";
                try {
                    const response = await fetch(item.link.href);
                    const fetchedText = await response.text();
                    iframe.srcdoc = fetchedText + magicScript;
                } catch (err) {
                    iframe.srcdoc = "<h3 style='font-family:Arial; padding: 20px; color: red;'>Erro ao carregar o arquivo.</h3>";
                }
            }
            
            viewBtn.innerText = '✔️ Lido';
        ...
        favBtn.innerText = '⭐ Marcar';
        favBtn.className = 'btn-marcar';
        favBtn.style.cssText = 'margin-left: 5px; background: #ffc107; color: black; border: none; padding: 2px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; font-weight: bold;';
        
        favBtn.onclick = (e) => {
            e.preventDefault();
            const isFav = item.row.getAttribute('data-fav') === 'true';
            if (isFav) {
                item.row.setAttribute('data-fav', 'false');
                item.row.style.backgroundColor = preFetchedText ? '#d4edda' : ''; 
                favBtn.innerText = '⭐ Marcar';
            } else {
                item.row.setAttribute('data-fav', 'true');
                item.row.style.backgroundColor = '#ffeeba'; 
                item.row.style.opacity = '1'; 
                favBtn.innerText = '🌟 Desmarcar';
            }
        };
        item.link.parentNode.appendChild(favBtn);
    };
    ...
    const performSearch = async () => {
        ...
        status.innerText = 'Mapeando arquivos...';
        ...
        document.querySelectorAll('.btn-visualizar, .btn-marcar').forEach(btn => btn.remove());

        const rowData = [];
        for (let link of links) {
            const row = link.closest('tr');
            if (row) {
                let rowDateText = row.cells && row.cells.length >= 4 ? row.cells[3].innerText.trim() : "";
                let rowDate = parseTasyDate(rowDateText);
                rowData.push({ link, row, rowDate });
            }
        }
        ...
        for (let data of rowData) {
            ...
            if (data.rowDate) {
                if (startDate && data.rowDate < startDate) matchesDate = false;
                if (endDate && data.rowDate > endDate) matchesDate = false;
            }

            if (matchesDate) {
                linksToFetch.push(data);
                data.row.style.display = ''; 
                data.row.style.backgroundColor = ''; 
                data.row.style.opacity = '1'; 
                data.row.removeAttribute('data-fav'); 
            } else {
                data.row.style.display = 'none';
            }
        }

        if (linksToFetch.length > 50 && !startDateStr && !endDateStr && keyword) {
            alert("Muitos arquivos encontrados. Por favor, preencha o Período para evitar travamentos no navegador.");
            status.innerText = "Busca bloqueada. Utilize o filtro de data.";
            return;
        }

        if (!keyword) {
             linksToFetch.forEach(item => { 
                 attachViewerAndButtons(item, null, null); 
             });
             status.innerText = `Filtro de data concluído! ${linksToFetch.length} arquivo(s).`;
             return;
        }

        status.innerText = 'Buscando e formatando... aguarde.';
            ...
            await Promise.all(batch.map(async (item) => {
                try {
                    ...
                    if (text.toLowerCase().includes(keyword)) {
                        ...
                        foundCount++;
                        attachViewerAndButtons(item, text, keyword);
                    } else {
                ...
                processedCount++;
                status.innerText = `Analisando: lote atual... (${processedCount}/${linksToFetch.length})`;
            }));
        }
